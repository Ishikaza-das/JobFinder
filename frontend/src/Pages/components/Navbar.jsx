  ...
  return (
    ...
    <div className="flex gap-6">
        {Navlist.map((item, index) =>(
          <Link to={item.path} key={index} onClick={() => setActiveItem(item.path)} className={`cursor-pointer text-xl font-serif transition-colors
            ${activeItem === item.path ? 'text-blue-600' : 'hover:text-blue-600'}`}>{item.name}</Link>
        ))}
    </div>
